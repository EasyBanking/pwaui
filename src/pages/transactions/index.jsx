          ...
          data={
            transactions.filter( (transaction) =>{
              console.log(searchFilter)
                ...
                else if(JSON.stringify(transaction).toLowerCase().includes(searchFilter.toLowerCase()))
                
                return transaction
              }
            ).map((transaction,key)=>{
              return {
                Transaction_ID: <Link key ={key} to="/locations">{transaction._id}</Link>,
                Transaction_Status:transaction.status,
                 Type: transaction.type,
                 Sender: <Link to="/locations">{transaction.SenderId._id}</Link>,
                 Sender_Username: transaction.SenderId.username,
                 Receiver: <Link to="/locations">{transaction.ReceiverId._id}</Link>,
